...
# Bz=3μG、eta_ohm=0.03、自己重力OFF

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
ATHENA="${ATHENA_DIR:-$(cd -- "${SCRIPT_DIR}/.." && pwd)}"
WORK="${WORK_DIR:-/work/beta/aida}"
OUTDIR="${OUTPUT_DIR:-${WORK}/results/Toyouchi-test52}"  #毎回変更
INPUT="${INPUT_FILE:-${ATHENA}/inputs/hydro/Toyouchi_test/athinput.Toyouchi_44}"  #毎回変更
