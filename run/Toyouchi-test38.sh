...
# commit: 002ad4a //本シミュレーションに対応するToyouchi.cppをgitの履歴から追跡可
# 計算領域をもとのサイズ448^3に戻し、test37と同設定で比較テスト

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
ATHENA="${ATHENA_DIR:-$(cd -- "${SCRIPT_DIR}/.." && pwd)}"
WORK="${WORK_DIR:-/work/beta/aida}"
OUTDIR="${OUTPUT_DIR:-${WORK}/results/Toyouchi-test38}"  #毎回変更
INPUT="${INPUT_FILE:-${ATHENA}/inputs/hydro/Toyouchi_test/athinput.Toyouchi_30}"  #毎回変更
