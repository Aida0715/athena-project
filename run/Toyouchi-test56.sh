...
# 磁場なし、回転のみ、自己重力ON。密度、シンクフロアをともに1e-6にした。

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
ATHENA="${ATHENA_DIR:-$(cd -- "${SCRIPT_DIR}/.." && pwd)}"
WORK="${WORK_DIR:-/work/beta/aida}"
OUTDIR="${OUTPUT_DIR:-${WORK}/results/Toyouchi-test56}"  #毎回変更
INPUT="${INPUT_FILE:-${ATHENA}/inputs/hydro/Toyouchi_test/athinput.Toyouchi_47}"  #毎回変更
